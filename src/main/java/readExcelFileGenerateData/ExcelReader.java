...
public class ExcelReader {
    ...
    private static ExcelData convertRowToData(Row row) {
        ...
        String background = convertCellValueToString(cell);
        resultData.setBackground(background);
        // 场景
        cell = row.getCell(cellNum++);
        String scenes = convertCellValueToString(cell);
        if (null == scenes || "".equals(scenes)) {
            resultData.setScenes(null);
        } else {
            resultData.setScenes(scenes);
        }
        // 获取目标
        cell = row.getCell(cellNum++);
        String target = convertCellValueToString(cell);
        resultData.setTarget(target);
        // 获取要点
        cell = row.getCell(cellNum++);
        ...
        // 获取习惯
        cell = row.getCell(cellNum++);
        String habit = convertCellValueToString(cell);
        if (habit.equals("1")) {
            resultData.setHabit(true);
        } else if (habit.equals("0")) {
            resultData.setHabit(false);
        }
        // 行为
        cell = row.getCell(cellNum++);
        String behavior = convertCellValueToString(cell);
        resultData.setBehavior(behavior);
        // 价值
        cell = row.getCell(cellNum++);
